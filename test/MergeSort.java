...
public class MergeSort{
	...
	*/
	...
	public static ListNode findMid(ListNode head,ListNode end){
		...
		if (end == null||end.next == null||end.next.next==null){
			return mid;
		}
		end = end.next;
		return findMid(head.next,end.next);

	}

	public static ListNode lastNodeTraverser(ListNode head){
		if(head.next == null || head == null){
			return head;	
		}
		return lastNodeTraverser(head.next);
...
*/
	...
	public static ListNode MergeSort(ListNode head){
		...
		ListNode tail = middle.next; //break from middle pointer.next to make right 
		
		middle.next = null;
		
		ListNode left =  MergeSort(head); //recursively create lists of size one
		ListNode right = MergeSort(tail); //by continuously splitting in half.
